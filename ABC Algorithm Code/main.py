...
def load_problem_data():
    ...
    # Vehicle capacities
    P = {d: 3000 for d in VD}  # Primary vehicle capacities
    Q = {s: {d: 1000 for d in VD} for s in VS}  # Secondary vehicle compartment capacities
    W = {s: 20000 for s in VS}  # Satellite capacities
...
def print_solution_summary(solution, run_number):
    """Print a brief summary of the solution"""
    try:
        stats = solution.get_route_statistics()
        validation = solution.validate_solution()
        print(f"   Run {run_number:2d}: Fitness = {solution.fitness:8.4f} | "
              f"Served = {stats['customers_served']:2d}/{stats['total_customers']:2d} | "
              f"Valid = {'✅' if validation['is_valid'] else '❌'}")
    except Exception as e:
        print(f"   Run {run_number:2d}: Error getting solution details - {e}")


def run_multiple_experiments(problem, best_params, n_runs=25):
    """Run ABC algorithm multiple times with fixed best parameters"""
    print(f"\n🚀 Starting {n_runs} experimental runs")
    ...
    for run_number in range(1, n_runs + 1):
        try:
            ...
            stats = best_solution.get_route_statistics()
            ...
            run_result = {
                ...
                'convergence_curve': [float(x) for x in convergence_data],
                'total_iterations': len(convergence_data),
                'improvements_count': getattr(abc, 'improvements_count', 0),
                'customers_served': stats['customers_served'],
                'total_customers': stats['total_customers'],
                'service_rate': stats.get('service_rate', 0),
                'is_valid': validation['is_valid'],
                'violations': validation.get('violations', []),
                'parameters_used': best_params.copy()
            }
            ...
            # Print run summary
            print_solution_summary(best_solution, run_number)

        except Exception as e:
            print(f"   Run {run_number:2d}: ❌ Failed - {str(e)[:50]}...")
            continue
    ...
    if successful_runs > 0:
        ...
        summary_stats = {
            ...
            'fitness_values_list': [float(f) for f in fitness_values]  # For research paper
        }

        print(f"📊 EXPERIMENT SUMMARY:")
        print(f"   Successful runs: {successful_runs}/{n_runs} ({summary_stats['success_rate']:.1%})")
        print(f"   Best fitness: {summary_stats['best_fitness']:.4f}")
        print(f"   Mean fitness: {summary_stats['mean_fitness']:.4f} ± {summary_stats['std_fitness']:.4f}")
        print(f"   Median fitness: {summary_stats['median_fitness']:.4f}")
        print(f"   Mean execution time: {summary_stats['mean_execution_time']:.2f}s")
        print(f"   Total time: {summary_stats['total_execution_time']:.1f}s")
        ...
        final_results = {
            'experiment_info': {
                ...
                'problem_size': {
                    'depots': len(problem.VD),
                    'satellites': len(problem.VS),
                    'customers': len(problem.VC)
                }
            },
        ...
        return final_results, fitness_values
...
def create_results_visualization(fitness_values, convergence_data=None, save_plots=True):
    ...
    # Plot 1: Fitness values across runs
    ax1 = plt.subplot(2, 2, 1)
    plt.plot(range(1, len(fitness_values) + 1), fitness_values, 'o-',
             color='blue', markersize=6, linewidth=2)
    plt.title('Fitness Values Across Runs', fontsize=12, fontweight='bold')
    ...
    plt.hist(fitness_values, bins=min(10, len(set(fitness_values))),
             alpha=0.7, color='green', edgecolor='black')
    plt.title('Distribution of Fitness Values', fontsize=12, fontweight='bold')
    ...
    box_plot = plt.boxplot(fitness_values, patch_artist=True)
    box_plot['boxes'][0].set_facecolor('lightblue')
    box_plot['boxes'][0].set_alpha(0.7)
    plt.title('Fitness Values Box Plot', fontsize=12, fontweight='bold')
    plt.ylabel('Fitness Value')
    plt.grid(True, alpha=0.3)

    # Plot 4: Convergence curve (if available)
    ax4 = plt.subplot(2, 2, 4)
    if convergence_data and len(convergence_data) > 0:
        plt.plot(convergence_data, 'r-', linewidth=2)
        plt.title('Convergence Curve (Last Run)', fontsize=12, fontweight='bold')
        ...
        plt.title('Convergence Curve', fontsize=12, fontweight='bold')

    plt.tight_layout()

    if save_plots:
        plot_filename = f'experiment_results_{timestamp}.png'
...
def main():
    """Main function to run 30 experiments with best hyperparameters"""
    try:
        print("🔬 MDMS-MCVRP Multiple Runs Experiment")
        print("=" * 50)
        ...
        print(f"Problem loaded: {len(VD)} depots, {len(VS)} satellites, {len(VC)} customers")
        if hasattr(problem, 'print_problem_summary'):
            problem.print_problem_summary()

        # *** UPDATE THESE WITH YOUR BEST HYPERPARAMETERS FROM GRID SEARCH/TPE ***
        best_hyperparameters = {
            'swarm_size': 80,  # Replace with your best value
            'max_iterations': 450,  # Replace with your best value
            'employed_ratio': 0.4,  # Replace with your best value
            'onlooker_ratio': 0.25,  # Replace with your best value
            'limit': 35  # Replace with your best value
        }

        # Run 25 experiments
        print(f"\n🎯 Running 30 experiments with your best hyperparameters...")
        results, fitness_values = run_multiple_experiments(
            problem,
            best_hyperparameters,
            n_runs=30
        )

        if results and fitness_values:
            # Save results to JSON
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            results_filename = f'25_runs_results_{timestamp}.json'

            if save_results_to_json(results, results_filename):
                print(f"\n💾 Results saved to: {results_filename}")

                # Print fitness values for research paper
                print(f"\n📝 FITNESS VALUES FOR YOUR RESEARCH PAPER:")
                print("=" * 50)
                fitness_list = results['summary_statistics']['fitness_values_list']
                print("Fitness values:", [f"{f:.4f}" for f in fitness_list])
                print(f"\nSummary Statistics:")
                print(f"Best: {results['summary_statistics']['best_fitness']:.4f}")
                print(f"Mean: {results['summary_statistics']['mean_fitness']:.4f}")
                print(f"Std: {results['summary_statistics']['std_fitness']:.4f}")
                print(f"Median: {results['summary_statistics']['median_fitness']:.4f}")

                # Create visualizations
                last_convergence = results['individual_runs'][-1]['convergence_curve'] if results[
                    'individual_runs'] else None
                create_results_visualization(fitness_values, last_convergence, save_plots=True)

                print(f"\n✅ Experiment completed successfully!")
                print(f"Check '{results_filename}' for detailed results.")
