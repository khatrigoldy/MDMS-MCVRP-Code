...
def load_problem_data():
    """Load the problem data with your nested dictionary format"""
    ...
    # PASTE YOUR COMPLETE distances DICTIONARY HERE
    ...
    # PASTE YOUR COMPLETE demands DICTIONARY HERE
    demands = {
    ...
    # Base capacities
    P = {d: 3000 for d in VD}  # Primary vehicle capacities (FIXED - NOT VARIED)
    Q = {s: {d: 1000 for d in VD} for s in VS}  # Secondary vehicle compartment capacities (TO BE VARIED)
    W = {s: 20000 for s in VS}  # Satellite capacities (TO BE VARIED)

    return VD, VS, VC, distances, demands, P, Q, W


def run_sensitivity_scenario(problem, params, satellite_cap, secondary_cap, n_runs=5):
    """Run multiple runs for a specific capacity combination"""
    fitness_values = []
    execution_times = []
    successful_runs = 0

    # Update problem capacities
    problem.W = {s: satellite_cap for s in problem.VS}
    problem.Q = {s: {d: secondary_cap for d in problem.VD} for s in problem.VS}

    print(f"  Testing: Satellite={satellite_cap}, Secondary={secondary_cap}")
    ...
    for run in range(1, n_runs + 1):
        try:
            ...
            # Run ABC algorithm
            abc = MDMSMCVRP_ABC(problem, **params)
            best_solution, convergence_data = abc.optimize()

            execution_time = time.time() - start_time

            # Validate solution
            validation = best_solution.validate_solution()
    ...
    if successful_runs > 0:
        return {
            'satellite_capacity': satellite_cap,
            'secondary_capacity': secondary_cap,
            'successful_runs': successful_runs,
            'total_runs': n_runs,
            'success_rate': successful_runs / n_runs,
            'best_fitness': float(min(fitness_values)),
...
def run_sensitivity_analysis(problem, best_params, n_runs_per_scenario=5):
    """Run comprehensive sensitivity analysis"""
    print("\n🔬 STARTING SENSITIVITY ANALYSIS")
    print("=" * 70)

    # Define capacity variations (±20% from baseline)
    satellite_capacities = [19500, 20000, 21000, 22000, 23000]  # ±12.5% around baseline
    secondary_capacities = [800, 900, 1000, 1100, 1200]

    print(f"📊 Satellite Capacities: {satellite_capacities}")
    print(f"📊 Secondary Capacities: {secondary_capacities}")
    print(f"📊 Total Scenarios: {len(satellite_capacities) * len(secondary_capacities)}")
    print(f"📊 Runs per scenario: {n_runs_per_scenario}")
    print(f"📊 Total runs: {len(satellite_capacities) * len(secondary_capacities) * n_runs_per_scenario}")
    print()

    all_results = []
    scenario_count = 0
    total_scenarios = len(satellite_capacities) * len(secondary_capacities)

    for satellite_cap in satellite_capacities:
        for secondary_cap in secondary_capacities:
            scenario_count += 1
            print(f"🎯 Scenario {scenario_count}/{total_scenarios}")

            result = run_sensitivity_scenario(
                problem, best_params, satellite_cap, secondary_cap, n_runs_per_scenario
            )
            ...
            if result['successful_runs'] > 0:
                print(f"  ✅ Success: {result['successful_runs']}/{result['total_runs']} runs")
                print(f"     Mean Fitness: {result['mean_fitness']:.2f} ± {result['std_fitness']:.2f}")
            else:
...
def create_sensitivity_table_data(results):
    """Create table data for research paper"""
    table_data = []

    for result in results:
        if result['successful_runs'] > 0:
            row = {
                'Satellite_Capacity': result['satellite_capacity'],
                'Secondary_Capacity': result['secondary_capacity'],
                'Success_Rate': f"{result['success_rate']:.1%}",
                'Best_Fitness': f"{result['best_fitness']:.2f}",
                'Mean_Fitness': f"{result['mean_fitness']:.2f}",
                'Std_Fitness': f"{result['std_fitness']:.2f}",
                'Mean_Time': f"{result['mean_execution_time']:.2f}s"
            }
        else:
            row = {
                'Satellite_Capacity': result['satellite_capacity'],
...
def save_sensitivity_results(results, table_data):
    """Save sensitivity analysis results"""
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')

    # Save detailed results
    detailed_filename = f'sensitivity_analysis_detailed_{timestamp}.json'
    with open(detailed_filename, 'w') as f:
        json.dump({
            'experiment_info': {
                'timestamp': datetime.now().isoformat(),
                'analysis_type': 'sensitivity_analysis',
                'varied_parameters': ['satellite_capacity', 'secondary_capacity'],
                'fixed_parameters': ['primary_capacity'],
            },
            'results': results
        }, f, indent=2)

    # Save table data (research paper ready)
    table_filename = f'sensitivity_table_{timestamp}.json'
    with open(table_filename, 'w') as f:
        json.dump({
            'table_headers': ['Satellite_Capacity', 'Secondary_Capacity', 'Success_Rate',
                              'Best_Fitness', 'Mean_Fitness', 'Std_Fitness', 'Mean_Time'],
    ...
    return detailed_filename, table_filename


def create_sensitivity_visualization(results):
    """Create visualization for sensitivity analysis"""
    # Extract data for plotting
    satellite_caps = sorted(list(set([r['satellite_capacity'] for r in results])))
    secondary_caps = sorted(list(set([r['secondary_capacity'] for r in results])))

    # Create fitness heatmap
    fitness_matrix = np.zeros((len(secondary_caps), len(satellite_caps)))

    for i, sec_cap in enumerate(secondary_caps):
        for j, sat_cap in enumerate(satellite_caps):
            result = next((r for r in results if
                           r['satellite_capacity'] == sat_cap and
                           r['secondary_capacity'] == sec_cap), None)
            if result and result['mean_fitness'] is not None:
                fitness_matrix[i, j] = result['mean_fitness']
            else:
    ...
    # Heatmap
    plt.subplot(2, 2, 1)
    im = plt.imshow(fitness_matrix, cmap='viridis', aspect='auto')
    plt.colorbar(im, label='Mean Fitness')
    plt.xlabel('Satellite Capacity')
    plt.ylabel('Secondary Vehicle Capacity')
    plt.title('Sensitivity Analysis Heatmap')
    plt.xticks(range(len(satellite_caps)), satellite_caps)
    plt.yticks(range(len(secondary_caps)), secondary_caps)

    # Success rate plot
    plt.subplot(2, 2, 2)
    success_rates = [r['success_rate'] for r in results if r['success_rate'] is not None]
    plt.hist(success_rates, bins=10, alpha=0.7, color='green')
    ...
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    plot_filename = f'sensitivity_analysis_{timestamp}.png'
...
def main():
    ...
    try:
        print("🔬 MDMS-MCVRP SENSITIVITY ANALYSIS")
        print("=" * 50)
        ...
        print(f"Problem: {len(VD)} depots, {len(VS)} satellites, {len(VC)} customers")

        # Best hyperparameters from your previous optimization
        ...
        print(f"Using parameters: {best_hyperparameters}")

        # Run sensitivity analysis
        results = run_sensitivity_analysis(problem, best_hyperparameters, n_runs_per_scenario=5)

        # Create table data
        table_data = create_sensitivity_table_data(results)

        # Save results
        detailed_file, table_file = save_sensitivity_results(results, table_data)

        # Print summary for research paper
        print("\n📝 SUMMARY FOR RESEARCH PAPER:")
        print("=" * 50)

        successful_scenarios = [r for r in results if r['successful_runs'] > 0]
        if successful_scenarios:
            all_fitness = []
            for r in successful_scenarios:
                all_fitness.extend(r['fitness_values'])

            print(f"Total scenarios tested: {len(results)}")
            print(f"Successful scenarios: {len(successful_scenarios)}")
            print(f"Overall success rate: {len(successful_scenarios) / len(results):.1%}")
            print(f"Overall best fitness: {min(all_fitness):.2f}")
            print(f"Overall worst fitness: {max(all_fitness):.2f}")
            print(f"Overall mean fitness: {np.mean(all_fitness):.2f} ± {np.std(all_fitness):.2f}")

        # Create visualization
        create_sensitivity_visualization(results)

        print(f"\n✅ Sensitivity analysis completed!")
        print(f"📊 Check '{table_file}' for research paper table data")
